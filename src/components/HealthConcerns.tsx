...
export function HealthConcerns() {
  ...
  return (
    <section className="bg-[#fcfaf8] py-12 md:py-16">
      <div className="mx-auto grid max-w-6xl grid-cols-1 items-start gap-6 sm:gap-8 px-4 sm:px-6 md:grid-cols-2">
        <div className="pt-2">
          <h2 className="text-[clamp(1.75rem,4vw,3rem)] leading-tight font-semibold text-neutral-900 [text-wrap:balance]">
            Most healthcare waits for something to go wrong
          </h2>
          <p className="mt-3 max-w-md text-sm sm:text-base text-neutral-600">
          ...
          <div className="relative max-h-[300px] sm:max-h-[400px] md:max-h-[500px] overflow-hidden pr-1 touch-none">
            ...
            >
              {[...concerns, ...concerns, ...concerns].map((concern, index) => (
                <li 
                  key={`${concern}-${index}`}
                  className="flex items-center rounded-2xl border border-zinc-100 bg-white px-4 py-4 sm:px-6 sm:py-5 shadow-[0_1px_0_rgba(0,0,0,0.04)]"
                >
                  <span 
                    className="mr-3 inline-flex h-2.5 w-2.5 items-center justify-center rounded-full flex-shrink-0" 
                    style={{backgroundColor: '#87CEEB', boxShadow: '0 0 0 4px rgba(135,206,235,0.25)'}}
                    aria-hidden="true"
                  ></span>
                  <span className="text-lg sm:text-xl font-medium leading-none text-zinc-800">{concern}</span>
                </li>
              ))}
            </ul>
          </div>
